...
# Nearest live option expiry (skip today)
opt_expiry = next(
    (
        e for e in sorted({
...
# ── Build order list ──────────────────────────────────────────────────────────
print(f"\n[3] Orders to be placed (BUY LIMIT, qty={QTY} each)")
print()
...
orders: list[dict] = []
for K in STRIKES:
    pe_instr = opts.get(("PE", K))
    if pe_instr is None:
        print(f"  {K:>7}  *** NOT FOUND IN INSTRUMENTS ***")
        continue
    theory = b76_price('p', F, K, T, RISK_FREE, iv)
    limit  = round_tick(theory * 0.50)
    total  = limit * QTY
    print(f"  {K:>7}  {pe_instr['tradingsymbol']:>28}  {theory:>8.2f}  {limit:>8.2f}  {total:>10.2f}")
    orders.append({
        "tradingsymbol": pe_instr["tradingsymbol"],
        "strike": K,
        "limit_price": limit,
        "theory": theory,
    })
...
if not orders:
    print("\nERROR: No instruments found for any strike. Aborting.")
    sys.exit(1)
...
# ── Place orders ──────────────────────────────────────────────────────────────
print(f"\n[4] Placing {len(orders)} BUY LIMIT orders...")
results = []
for o in orders:
    try:
